...
> **Total:** 9 test cases - 3 Positive | 3 Negative | 3 Edge
...
| TC-SALE-005 | P2 | Negative | Sale tag is not shown when discount pricing payload is incomplete | AC-1 |
...
| TC-SALE-008 | P3 | Edge | Equal original and current prices do not trigger Sale tag | AC-1, AC-2 |
| TC-SALE-009 | P3 | Edge | Infinite scroll keeps Sale-tag rule consistent for mixed products | AC-1, AC-2 |
...
- Feature controls discount visualization in a storefront product journey.
- Primary user is a shopper browsing listings and opening product detail pages.
- Business goal is to highlight discounted catalog items while avoiding redundant badge noise on detail pages.

AC decomposition:
- AC-1: Discounted products on listing pages must display the "Sale" tag.
- AC-2: Non-discounted products must not display the "Sale" tag.
- AC-3: Discounted products on view-item page must show strike-through pricing only, without "Sale" tag.

Coverage mapping:
- Positive: Correct badge visibility and pricing presentation for normal browsing.
- Negative: Prevent false-positive badges when pricing payload is incomplete or UI state leaks between pages.
- Edge: Validate boundary discounts, equal-price conditions, and mixed catalogs under pagination/scroll.

Priority rationale:
- P1: Core merchandising behavior on listing and detail pages.
- P2: Data integrity and state-isolation failures that can mislead shoppers.
- P3: Boundary and continuity behavior impacting real but less frequent combinations.

Visual context:
- No screenshots were attached in this run; coverage is derived from issue context and prior accepted AC interpretation.
-->
...
## TC-SALE-005 | P2 | Negative | Sale tag is not shown when discount pricing payload is incomplete
...
```gherkin
Scenario: Sale tag is not shown when discount pricing payload is incomplete
  Given an anonymous user opens the Home page
  When a product card payload contains discounted price "$0.00" and missing original price
  Then the product card should not display the tag "Sale"
  And the UI should show only the valid current price without strike-through
```

**Notes:** Verifies the badge depends on valid reduction data, not partial malformed payloads.
---
...
## TC-SALE-008 | P3 | Edge | Equal original and current prices do not trigger Sale tag
...
```gherkin
Scenario: Equal original and current prices do not trigger Sale tag
  Given an anonymous user opens the Home page
  When a product card shows original price "$75.00" and discounted price "$75.00"
  Then the product card should not display the tag "Sale"
  And only one effective product price should be emphasized to the user
```

---

## TC-SALE-009 | P3 | Edge | Infinite scroll keeps Sale-tag rule consistent for mixed products
...
|----|-------------|----------|
| AC-1 | Discounted products on page should have "Sale" tag | TC-SALE-001, TC-SALE-005, TC-SALE-007, TC-SALE-008, TC-SALE-009 |
| AC-2 | Non-discounted products should not have "Sale" tag | TC-SALE-002, TC-SALE-004, TC-SALE-008, TC-SALE-009 |
